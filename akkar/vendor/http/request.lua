...
local client = require "akkar.vendor.http.client"
local new_headers = require "akkar.vendor.http.headers".new
local http_cookie = require "akkar.vendor.http.cookie"
local http_hsts = require "akkar.vendor.http.hsts"
local http_proxies = require "akkar.vendor.http.proxies"
local http_util = require "akkar.vendor.http.util"
local http_version = require "akkar.vendor.http.version"
...
function request_methods:go(timeout)
	...
	if proxy then
		...
		elseif proxy.scheme:match "^socks" then
			-- SOCKS is not carried in this build. It was the other module
			-- pulling in the `compat53` rock, which is the shim that stops
			-- Lua 5.5, and an outbound API client does not need it. The HTTP
			-- proxy branch above still works.
			error("this build does not support SOCKS proxies")
		else
