...
--- so never called the yield `yielding` handed it, which made `chunked(10)`
--- over a million iterations produce **zero** scheduler trips while
--- documenting itself as "wraps a handler so its whole body runs with a yield
--- budget".  A wrapper that measurably does nothing is worse than no wrapper:
--- someone reads the call site and stops looking.
---
--- The old claim -- a yield budget over a body that does not cooperate --
--- cannot be honoured, and the header of this module already says why: there
--- is no point at which Lua regains control inside a C call that does not
--- return.  So the handler is given the yield and decides where a turn is
--- safe to take.
