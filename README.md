...
| | |
|---|---|
| Pooled Postgres connections keep session state unless asked to reset | A `SET search_path`, a temp table or a session GUC survives the connection's return to the pool and reaches the next checkout. Measured. It does **not** break `akkar.scope`, which isolates by rewriting the query and never touches session state, so this bites only applications that isolate with `SET ROLE`, RLS or a per-tenant `search_path`. `db.connect { reset_on_release = true }` runs `DISCARD ALL` on return; it is off by default because it costs a round-trip (167 µs, 4.2% of a real query) that most applications do not need |
