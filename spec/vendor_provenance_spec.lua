...
local PATCHES = {
  ...
  { file = "h2_stream.lua", commit = "uncommitted",
    needle = "RST_STREAM_BURST",
    why = "CVE-2023-44487 rapid reset: RST frees the stream slot, so MAX_CONCURRENT_STREAMS never fires" },
  { file = "h2_stream.lua", commit = "uncommitted",
    needle = "max_header_lines = 100",
    why = "h1 caps header lines at 100 and h2 capped nothing; the number must not drift apart" },
  { file = "h2_stream.lua", commit = "uncommitted",
    needle = "max_header_bytes = 32*1024",
    why = "the compressed frame cap does not bound the bytes expanded from HPACK" },
  { file = "hpack.lua", commit = "uncommitted",
    needle = "max_entries",
    why = "an indexed header field is one byte and appends a whole pair; 400 KB compressed reaches ~400,000 fields" },
  { file = "hpack.lua", commit = "uncommitted",
    needle = "header list of more than %d bytes",
    why = "HPACK repetition can expand a small block into an unbounded decoded header list" },
  ...
  { file = "websocket.lua", commit = "uncommitted",
    needle = "read_frame(sock, deadline, max_payload)",
    why = "a partial extended-length read must not drop the pre-buffer ceiling" },
  { file = "websocket.lua", commit = "uncommitted",
    needle = "A ping is proof that the peer is",
    why = "protocol heartbeats are activity for the runtime idle timeout" },
  ...
  { file = "h1_stream.lua", commit = "uncommitted",
    needle = "header_bytes_in_progress",
    why = "HTTP/1 request header bytes are bounded across partial reads" },
  { file = "h1_connection.lua", commit = "uncommitted",
    needle = "return key, val, nil, #line",
    why = "wire bytes must be charged before optional whitespace is trimmed" },
