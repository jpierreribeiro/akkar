...
wrapper passes its own arguments through and returns whatever the wrapped
function returns.

```lua no-run
local slow = work.chunked(500)(function(rows) return render(rows) end)
```

**The wrapper does not yield.** The module docstring says it makes a function's
"whole body run with a yield budget", and the source disagrees with it: the
wrapped function is called without the `yield` the budget is spent through, so
nothing inside it can reach the scheduler. Measured below, a neighbouring
coroutine makes no progress at all while a `chunked` function runs.

Until that is settled, thread [`work.yielding`](#workyieldingevery-fn) into the
loop by hand.
...
loop:wrap(function()
  ...
  local total = work.chunked(1)(function(n)
    local sum = 0
  ...
  print("neighbour ran", neighbour) --> 0
end)
assert(loop:loop())
```
